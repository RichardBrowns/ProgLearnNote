...
**教程：https://refactoringguru.cn/design-patterns/catalog**
...
行为设计模式（Behavioral Design Patterns）是一类在软件设计中专注于解决对象间交互、职责分配以及算法的封装与委托的问题的设计模式。它们的共性主要包括以下几个方面：

1. **关注点**：行为型设计模式主要关注的是程序在运行时对象之间的动态交互，以及如何在对象之间分配行为和责任，以支持复杂业务逻辑的实现。
2. **松耦合**：这类模式倾向于促进系统的松耦合，通过定义清晰的接口和协议来减少对象之间的直接依赖，使得对象能够更加独立地变化和演化。
3. **可扩展性**：通过将算法、状态改变或责任分配策略封装在独立的类或对象中，行为型模式允许系统更容易地添加新功能或改变已有行为，而不需要大规模修改现有的代码。
4. **代码重用**：许多行为模式提供了机制来复用行为相关的代码，比如通过策略模式可以轻松交换算法，迭代器模式允许以一致的方式遍历不同类型的集合。
5. **通信与协调**：这些模式常常涉及对象间的消息传递或协作策略，以协调复杂的操作流程，如责任链模式、观察者模式和中介者模式等。
6. **抽象层次**：行为模式不仅关注类和对象的结构，还深入到它们之间的交互模式，这要求设计者从更高的抽象层面考虑问题，以达到更好地解耦和灵活的设计。
7. **控制流**：行为模式经常涉及到对程序控制流的管理，例如决定何时何地执行某些操作，以及如何响应特定的事件或条件变化。

综上所述，行为型设计模式的核心目标在于优化和管理对象之间的交互，使系统在面对复杂行为和变化时，能够保持结构清晰、易于理解和维护。

## 行为型 - 观察者模式（Observer ）

观察者模式（Observer Pattern）是一种行为设计模式，它定义了对象之间的一对多依赖关系，当一个对象的状态发生改变时，所有依赖于它的对象都会得到通知并被自动更新。这种模式通常被用于实现事件处理系统、实时数据更新等场景。

**角色介绍**

1. **Subject（主题/被观察者）**：它是一个接口或抽象类，定义了添加观察者、删除观察者和通知观察者的方法。Subject维护着一个观察者列表，当其状态发生变化时，会通知所有的观察者。
2. **ConcreteSubject（具体主题）**：它是Subject的实现类，包含具体的状态，并负责在状态变化时通知所有注册的观察者。
3. **Observer（观察者）**：它是一个接口或抽象类，定义了一个更新的接口，用于接收主题的通知。
4. **ConcreteObserver（具体观察者）**：它是Observer的实现类，实现更新接口以定义当接收到主题通知时应进行的具体操作。

**工作流程**

1. **创建Subject类**：定义添加、删除观察者以及通知观察者的方法。
2. **创建Observer接口**：声明一个更新方法，用于 Observer 接收到消息后的更新操作。
3. **创建ConcreteSubject类**，继承或实现Subject，并维护观察者集合，具体实现通知观察者的方法。
4. **创建ConcreteObserver类**，实现Observer接口，定义具体的更新逻辑。
5. **注册观察者**：将 ConcreteObserver 实例添加到 ConcreteSubject 的观察者集合中。
6. **更改主题状态**：当 ConcreteSubject 的状态发生改变时，调用通知方法，遍历观察者集合，调用每个观察者的更新方法。

**优点**

- **松耦合**：Subject 和 Observer 之间的联系只存在于抽象层，具体主题和具体观察者之间解耦，两者可以独立地变化和复用。
- **开放封闭原则**：增加新的观察者无需修改原有代码，满足软件设计的开放封闭原则。
- **灵活性和可重用性**：观察者模式支持广播通信，方便系统扩展，同时观察者和被观察者都可以灵活地应用于不同的场景。

**缺点**

- 如果观察者数量众多，通知观察者可能会花费较多时间，影响系统性能。
...
class ConcreteSubject implements Subject {
    ...
    // 假设这是业务逻辑的一部分，状态改变后通知观察者
    public void changeState() {
        System.out.println("ConcreteSubject: Hey, my state has changed!");
...
public class ObserverPatternDemo {
    public static void main(String[] args) {
        ...
        subject.changeState();  // 这将触发观察者的update方法
    }
}
```
